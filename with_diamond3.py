...
# == Константы (Кроссплатформенные) ==
# Определяем операционную систему для установки правильных путей
if os.name == 'nt':  # Если запуск на Windows (локально)
    DIAMOND_DEFAULT_PATH = r"C:\Users\diamond\diamond.exe"
    CUSTOM_TEMP_DIR = r"C:\Users\diamond_tempp"
else:                # Если запуск на Linux (Streamlit Cloud)
    DIAMOND_DEFAULT_PATH = "diamond"       # В Linux программа будет доступна глобально
    CUSTOM_TEMP_DIR = "/tmp/diamond_tempp" # Стандартная папка для временных файлов в Linux
...
def main():
    ...
    if not diamond_valid:
        ...
        st.sidebar.warning("Try specifying full path to diamond executable")
    else:
...
def validate_diamond_path(path):
    ...
    # На Linux "diamond" это глобальная команда, а не путь к файлу, поэтому проверка os.path.exists не нужна для глобальных команд
...
# ============================================================
# == ФУНКЦИЯ: FNA / FASTA → FAA Converter ==
# ============================================================
def show_fna_to_faa_converter():
    ...
    with col1:
        reading_frame = st.selectbox("Reading frame", options=[1, 2, 3], index=0)
    with col2:
        table = st.selectbox(
            "Genetic code table",
            options=[1, 2, 4, 11],
    ...
    with col3:
        mode = st.selectbox("Translation mode", options=["CDS mode", "Full translation"])

    min_protein_len = st.number_input("Minimum protein length (aa)", min_value=1, value=10)
